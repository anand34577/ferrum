...
-- dropped, since dropping it buys nothing but migration risk for a column
-- that's simply ignored from here on.
INSERT INTO ai_provider_models (id, provider_id, label, model_id, is_default, created_at)
SELECT md5(random()::text || clock_timestamp()::text || id), id, model, model, is_default, updated_at
FROM ai_providers
