...
func (s *Server) currentPreferences(r *http.Request, userID string) (userPreferences, error) {
	prefs := userPreferences{Theme: "system", Accent: "oxide"}
	var activeDashboardID sql.NullString
	err := s.db.QueryRowContext(r.Context(), `SELECT theme, accent, active_dashboard_id FROM user_preferences WHERE user_id = ?`, userID).
		Scan(&prefs.Theme, &prefs.Accent, &activeDashboardID)
...
func (s *Server) putPreferences(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := s.db.ExecContext(r.Context(), `INSERT INTO user_preferences (user_id, theme, accent, active_dashboard_id, updated_at) VALUES (?, ?, ?, ?, ?)
		ON CONFLICT (user_id) DO UPDATE SET theme = excluded.theme, accent = excluded.accent, active_dashboard_id = excluded.active_dashboard_id, updated_at = excluded.updated_at`,
		u.ID, current.Theme, current.Accent, activeDashboardID, time.Now().UTC().Format(time.RFC3339)); err != nil {
		s.writeError(w, http.StatusInternalServerError, err)
		return
	}
	// Only the fields actually sent are worth a log line — logging theme+accent
